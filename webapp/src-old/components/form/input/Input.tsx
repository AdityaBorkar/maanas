...
export default function Input(props: InputProps) {
	...
	return (
		<div className={`relative ${props.wrapperClass}`}>
			...
			{INPUT_TYPE === 'currency' && (
				<span className='text-text-4 absolute left-2 top-8 z-10 peer-focus:text-neutral-600'>
			...
			<input
				...
				className={`text-text-3 w-full rounded-md border border-neutral-800 bg-inherit px-3 py-2 placeholder:text-neutral-700 focus:outline-0 ${InputClass} ${props?.inputClass} ${
					props.icon && 'pl-16'
				}`}
				onKeyUp={(e) => {
					// @ts-ignore
